        ...
        <div style="display:flex; gap:0.5rem; align-items:center; flex-shrink:0;">
            <div style="position:relative; flex-grow:1;">
                ...
                <input type="text" id="searchBar" class="form-control" placeholder="Search products..."
                    style="padding-left:35px; width:100%; height:42px;">
            </div>
            <div style="position:relative; width:220px;">
                <i class="fa-solid fa-barcode"
                    style="position:absolute; left:12px; top:50%; transform:translateY(-50%); color:var(--accent-color);"></i>
                <input type="text" id="barcodeInput" class="form-control" placeholder="Scan Barcode..."
                    style="padding-left:35px; width:100%; height:42px; border-color:rgba(255,107,53,0.4);">
            </div>
            <button onclick="openModal('customItemModal')" class="btn btn-secondary"
                style="white-space:nowrap; height:42px; display:inline-flex; align-items:center; gap:0.35rem;">
        ...
        <div class="catalog-filters">
            ...
            <?php foreach ($categories as $cat): ?>
                <span class="filter-pill"
                    onclick="filterCategory(<?= $cat['id'] ?>, this)"><?= h($cat['category_name']) ?></span>
            <?php endforeach; ?>
        </div>
            ...
            <?php foreach ($products as $p): ?>
                ...
                <?php if ($has_vars): ?>
                    <?php foreach ($prod_variants[$p['id']] as $v):
                        ...
                        ?>
                        <div class="prod-card" data-category="<?= $p['category_id'] ?>"
                            ...
                            data-size="<?= htmlspecialchars($v['size'], ENT_QUOTES) ?>" data-price="<?= $price ?>"
                            ...
                            <?php if (!empty($p['image_path'])): ?>
                                <div
                                    style="width:100%; height:90px; border-radius:var(--border-radius-sm); overflow:hidden; border:1px solid var(--border-color); margin-bottom:0.25rem;">
                                    <img src="../<?= h($p['image_path']) ?>" alt="<?= h($p['product_name']) ?>"
                                        style="width:100%; height:100%; object-fit:cover;">
                                </div>
                            <?php else: ?>
                                <div
                                    style="width:100%; height:90px; border-radius:var(--border-radius-sm); background:var(--bg-control); display:flex; align-items:center; justify-content:center; color:var(--text-muted); border:1px solid var(--border-color); margin-bottom:0.25rem;">
                                    <i class="fa-solid fa-image" style="font-size:1.5rem;"></i>
                                </div>
                            <?php endif; ?>

                            <div
                                style="flex-grow:1; display:flex; flex-direction:column; justify-content:space-between; gap:0.25rem;">
                                <div>
                                    <span class="badge"
                                        style="background:rgba(255,107,53,0.08); color:var(--accent-color); font-size:0.7rem; padding:0.15rem 0.4rem; margin-bottom:0.25rem; display:inline-block;">
                                        <?= h($p['category_name']) ?>
                                    </span>
                                    <div class="prod-title"
                                        style="min-height:32px; overflow:hidden; text-overflow:ellipsis; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical;">
                                        <?= h($p['product_name']) ?> <span
                                            style="color:var(--text-secondary); font-size:0.8rem; font-weight:normal;">(<?= h($v['size']) ?>)</span>
                                    </div>
                                ...
                                <div style="display:flex; justify-content:space-between; align-items:center;">
                                    ...
                                    <span style="font-size:0.7rem; color:var(--text-muted);"><i class="fa-solid fa-plus-circle"></i>
                                        Add</span>
                                </div>
                            </div>
                        </div>
...
<div id="cartPreviewModal" class="modal">
    <div class="modal-content" style="max-width: 620px;">
        ...
        <h3
            style="margin-bottom: 1.5rem; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid var(--border-color); padding-bottom: 0.75rem;">
        ...
        <div id="modalCartItemsList"
            style="display: flex; flex-direction: column; gap: 0.75rem; max-height: 350px; overflow-y: auto; margin-bottom: 1.5rem; padding-right: 0.25rem;">
            <!-- Dynamic items list -->
        </div>

        <div
            style="display: flex; justify-content: space-between; align-items: center; border-top: 1px dashed var(--border-color); padding-top: 1rem; margin-bottom: 1.5rem;">
            <span style="font-weight: 600; color: var(--text-secondary);">Subtotal:</span>
            <span id="modalCartSubtotal"
                style="font-size: 1.3rem; font-weight: 800; color: var(--accent-color);">₹0.00</span>
        </div>

        <div style="display: flex; justify-content: flex-end; gap: 0.75rem;">
            <button type="button" onclick="closeModal('cartPreviewModal')" class="btn btn-secondary">Continue
                Shopping</button>
            <button type="button" onclick="goToCheckout()" class="btn btn-success"
                style="box-shadow: 0 4px 12px rgba(46, 213, 115, 0.2); font-weight:600;">
    ...
    <div class="modal-content">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" min="0" step="0.01" id="customItemPrice" class="form-control" placeholder="50.00"
                    required>
            </div>
            <div class="form-group">
...
<script>
    ...
    window.addEventListener('DOMContentLoaded', () => {
        ...
        // Auto-detects scanner input (fast) vs manual typing (slow).
        // If all characters arrive within 100ms → scanner paste → auto-add immediately.
        // If typing is slow → wait for Enter key.
        ...
        if (barcodeInput) {
            ...
            // Keep input focused unless another form element is active
            ...
            // Track typing speed on each keypress
            ...
            // On each input event, check if characters arrived at scanner speed
            barcodeInput.addEventListener('input', () => {
                ...
                // Slow (manual) typing — do nothing, wait for Enter
            });

            // Handle paste event (always treat as scanner input)
